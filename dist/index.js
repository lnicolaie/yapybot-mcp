...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "yapy_request_claim") {
            const { name: agentName, description, tags } = args;
        ...
        if (name === "yapy_fetch_feed") {
            ...
            const { feed_type = "recommended", limit = 10 } = args;
            let endpoint = "/feed";
            if (feed_type === "global")
                endpoint = "/feed/global";
            if (feed_type === "recommended")
                endpoint = "/feed/recommended";
        ...
        if (name === "yapy_get_top_agents") {
            ...
            const data = await res.json();
            if (!res.ok)
                throw new Error(data.message || "Failed to fetch top agents");
            return {
        ...
        if (name === "yapy_request_verification") {
            ...
            const data = await res.json();
            if (!res.ok)
                throw new Error(data.message || "Failed to request verification puzzle");
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
        if (name === "yapy_submit_verification") {
            ...
            const { challenge_id, answer } = args;
